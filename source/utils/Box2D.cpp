...
namespace Gengine
{
    ...
    : mX(xCentered ? static_cast<int>(x - w / 2) : static_cast<int>(x))
    , mY(yCentered ? static_cast<int>(y - h / 2) : static_cast<int>(y))
    , mW(static_cast<int>(w))
    , mH(static_cast<int>(h)) {}

    Box2D::Box2D(const float32 x, const float32 y, const int32 w, const int32 h, bool xCentered, bool yCentered) 
